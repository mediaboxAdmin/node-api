...
class Validation {
   ...
   async length(key, value, params) {
      ...
      if (min && !max && value.length < min) {
         ...
         this.setError(
            key,
            this.customMessages?.[key]?.length || `The value of this filed must be between ${min} and ${max}`,
         )
      }
   }
   ...
   async email(key, value) {
      ...
      if (!validEmail) {
         this.setError(key, this.customMessages?.[key]?.email || "Invalid email")
      }
   }
   ...
   async exists(key, value, params) {
      try {
         ...
         if (!row) {
            this.setError(key, this.customMessages?.[key]?.exists || `This field is not defined on ${tableName} table`)
         }
      } catch (error) {
         throw error
      }
   ...
   async unique(key, value, params) {
      try {
         ...
         if (row) {
            this.setError(
               key,
               this.customMessages?.[key]?.unique || `The ${columnName} must be unique on ${tableName} table`,
            )
         }
      } catch (error) {
         throw error
